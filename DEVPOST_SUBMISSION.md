...
FireGuard streams wildfire detections for a Williams Lake / Cariboo scenario, watches for high-FRP hotspots near evacuation zones, and starts the `fireguard_evacuation` workflow when the threat threshold is met.

The workflow checks nearby evacuation zones, ESS shelters, road events, route safety, and fire context. It then produces an evacuation brief and pushes map annotations for the hotspot, affected zone, shelters, blockages, and evaluated routes.
...
We also had to make the UI and workflow share one contract. The replay stream, threat payload, agent overlay, map annotation callback, action panel, and workflow trace all needed to agree on the same state.

## Accomplishments that we're proud of

FireGuard connects detection to decision without an operator manually copying coordinates between systems.

We are proud of the automatic threat-to-workflow handoff, the constrained evacuation tool sequence, the route what-if support, and the map annotation bridge that turns agent output into visible operational context.

We are also proud that the project has focused tests for workflow contracts, tool behavior, storage, retry behavior, project data, and Vertex integration.
...
Finally, we learned that the trace matters. A judge or operator should be able to see the exact checks the workflow performed before trusting the brief.

## What's next for Untitled

Next steps are to add routed driving directions, broaden the scenario set, add shelter capacity confirmation, strengthen action-card output, and extend the map overlay so route and zone recommendations stay visible after the workflow completes.
