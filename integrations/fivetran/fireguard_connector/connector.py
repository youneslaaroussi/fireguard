...
def firms_external_id(row: dict[str, Any]) -> str:
...
def firms_acquired_at(row: dict[str, Any]) -> str:
...
def _firms_stream_row(row: dict[str, Any], source: str, ingested_at: str, raw_extra: dict[str, Any] | None = None) -> dict[str, Any]:
    acquired_at = firms_acquired_at(row)
    raw = {**row}
    if raw_extra:
        raw.update(raw_extra)
    return {
        "external_id": firms_external_id(row),
        "source": source,
        "latitude": float_value(row.get("latitude")),
        "longitude": float_value(row.get("longitude")),
        "brightness": float_value(row.get("bright_ti4") or row.get("brightness")),
        "confidence": str(row.get("confidence", "unknown")),
        "frp": float_value(row.get("frp")),
        "scan": float_value(row.get("scan")),
        "track": float_value(row.get("track")),
...
def _firms_rows(configuration: dict[str, Any]) -> Iterable[dict[str, Any]]:
    ...
    try:
        ...
        log.warning(f"NASA FIRMS fetch failed; emitting stored historical FIRMS snapshot: {exc}")
        yield from _replay_fire_hotspots(str(exc))
        return
...
def _replay_fire_hotspots(reason: str | None = None) -> Iterable[dict[str, Any]]:
    ingested_at = now_iso()
    try:
        with FIRMS_SNAPSHOT_CSV.open(newline="") as file:
            rows = list(csv.DictReader(file))
    except FileNotFoundError:
        log.warning(f"Stored FIRMS snapshot is missing: {FIRMS_SNAPSHOT_CSV}")
        return
    for row in rows:
        yield _firms_stream_row(
            row,
            "NASA_FIRMS_HISTORICAL_SNAPSHOT",
            ingested_at,
            {
