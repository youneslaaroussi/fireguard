...
def _slim_context(context: dict) -> dict:
    """Minimal context fields needed for frontend graph node building."""
    return {
        "fires": [
            {"external_id": f["external_id"], "frp": f.get("frp", 0), "source": f.get("source", "")}
            for f in context.get("fires", [])[:3]
        ],
        "zones": [
            {"zone_id": z["zone_id"], "name": z.get("name", ""), "population": z.get("population", 0), "vulnerable_count": z.get("vulnerable_count", 0)}
            for z in context.get("zones", [])
        ],
        "road_events": [
            {"external_id": r["external_id"], "road_name": r.get("road_name", ""), "title": r.get("title", "")}
            for r in context.get("road_events", [])[:2]
        ],
    }
...
def _assessment_event_generator(
    ...
    try:
        ...
        event = trace.add(
            "observe", "get_incident_context",
            {"incident_id": incident_id},
            {"fires": len(context["fires"]), "perimeters": len(context["perimeters"]), "road_events": len(context["road_events"]), "zones": len(context["zones"]), "shelters": len(context["shelters"])},
            [doc["external_id"] for doc in context["fires"][:2]],
        )
        yield {"type": "step", "event": event, "slim_context": _slim_context(context)}

        memory_hits = store.search_text(["policies", "road_events", "fire_hotspots"], "unsafe route closure shelter in place approval")
        event = trace.add(
            "retrieve", "search_operational_memory",
        ...
        if gemini_result.get("status") == "completed" and gemini_result.get("decision"):
            ...
            if not plan_validation.valid:
                yield {"type": "thinking", "step": "repair", "message": "Repairing plan..."}
                repair_result = gemini.run_gemini_plan_decision(
                    store.settings, incident_id, operational_brief,
                    repair_errors=plan_validation.errors, previous_decision=gemini_decision,
                )
                gemini_tool_calls.extend(repair_result.get("tool_calls", []))
                event = trace.add(
                    "repair", "gemini_repair",
                    {"incident_id": incident_id, "model": store.settings.gemini_model},
                    repair_result, repair_result.get("tool_calls", []),
                )
                yield {"type": "step", "event": event}
                if repair_result.get("status") == "completed" and repair_result.get("decision"):
                    ...
                    repaired_validation = validate_gemini_plan_decision(repaired_decision, context, zone_risks, routes, repair_attempted=True)
                    event = trace.add(
                        "validate", "plan_validation",
            ...
            if repair_result.get("status") == "completed" and repair_result.get("decision"):
                repaired_decision = GeminiPlanDecision.model_validate(repair_result["decision"])
                plan_validation = validate_gemini_plan_decision(repaired_decision, context, zone_risks, routes, repair_attempted=True)
                validation_errors = list(plan_validation.errors)
                gemini_decision = repaired_decision.model_dump()
                event = trace.add(
                    "validate", "plan_validation",
        ...
        event = trace.add(
            ...
            plan_validation.model_dump(),
            [evidence_id for step in (gemini_decision or {}).get("steps", []) for evidence_id in step.get("evidence_ids", [])],
        )
        yield {"type": "step", "event": event}
        ...
        yield {"type": "step", "event": event}
        event = trace.add("approval", "request_human_approval", {"bundle_id": bundle_id}, approval.model_dump(), [])
        yield {"type": "step", "event": event}
        event = trace.add(
            "compile", "action_bundle_compile",
            {"bundle_id": bundle_id, "planning_mode": planning_mode},
            {"action_count": len(actions), "requested_action_count": len(plan.requested_actions), "approval_status": approval.status},
            [evidence_id for action in actions for evidence_id in action.evidence_ids],
            action_assumption_ids,
        )
        yield {"type": "step", "event": event}
        ...
        agent_review = {
            "status": gemini_result.get("status"), "model": gemini_result.get("model"),
            "planning_mode": planning_mode, "tool_calls": gemini_tool_calls,
            "decision": gemini_decision, "repair": repair_result,
        ...
        store.upsert("traces", trace_id, {
            "trace_id": trace_id, "incident_id": incident_id, "events": trace.events,
            "phoenix_trace_ids": trace.phoenix_trace_ids, "arize_ax_trace_ids": trace.arize_ax_trace_ids,
            "created_at": now_iso(),
        })
        evaluate_incident(store, incident_id)

        result = AssessmentResult(
            incident_id=incident_id, context=context, plan=plan, bundle_id=bundle_id,
            approval=approval, actions=actions, trace=trace.events, agent_review=agent_review,
            gemini_status=agent_review.get("status"), gemini_tool_calls=gemini_tool_calls,
            planning_mode=planning_mode,  # type: ignore[arg-type]
            gemini_decision=gemini_decision, plan_validation=plan_validation.model_dump(),
            validation_errors=validation_errors, candidate_facts_summary=candidate_facts_summary,
            phoenix_trace_ids=trace.phoenix_trace_ids, arize_ax_trace_ids=trace.arize_ax_trace_ids,
        )
...
def run_assessment(
    ...
    for event_dict in _assessment_event_generator(store, incident_id, commander_context=commander_context, custom_fires=custom_fires):
        ...
        if event_dict["type"] == "error":
            raise RuntimeError(event_dict["message"])
    raise RuntimeError("Assessment generator ended without a result")
